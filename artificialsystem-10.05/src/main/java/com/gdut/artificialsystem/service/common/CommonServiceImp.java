...
public class CommonServiceImp implements CommonService{
    ...
    public Msg resetPassword(String userId,String userPassword,String identity) {
        //1.输入判断
        //1.1判断用户id是否是数字
        if(StringUtil.notDigitalStr(userId))
            return new Msg(0,"Userid is wrong",null);

        //1.2判断是否空输入
        if(StringUtil.isEmptyStr(userPassword)||StringUtil.isEmptyStr(identity))
        ...
        //2.调用dao方法，检查userid对应的身份验证(identity)
        //2.2检查realidentity和identity是否一致
        User real=userMapper.selectById(Long.valueOf(userId));
        if(!real.getIdentityid().equals(user.getIdentityid()))
            return new Msg(0,"Identity message is wrong",null);

        //3.如果一致，更新用户的密码。并检查结果
        int result=userMapper.updateById(user);
        if(result>0)
            return new Msg(1,"Successfully reset your password",user);
        else
            return new Msg(0,"Sql error",null);
    }
